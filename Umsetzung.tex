...
\begin{figure}[h]
	\begin{center}
		...
		\caption[Funktionsweise Fliehkraftabscheider]{Funktionsweise eines klassischen Fliehkraftabscheiders, \cite{Zyklon2009}}
	\end{center}
\end{figure}
...
\begin{figure}[h]
	\begin{center}
		...
		\caption[Adapter für Ionisator]{Der Adapter wird auf den Ionisator geklebt und anschließend in den Anschlusszylinder eingedreht. Dabei drückt der hervorstehende Ring in die Nut den Anschlusszylinders/ noch das falsche Bild}
	\end{center}
\end{figure}
...
Man kann $d_2$, $d_3$ und $\alpha$ aus der Gewindetabelle im Anhang ablesen. Der Reibwert $\mu$ wird mit $\mu = \SI{1,3}{}$ angenommen \cite{Matek2011}. $\phi'$ berechnet man entsprechend der Formel:
\begin{align*}
	\phi' &= \arctan\left( \frac{1,3}{\cos(\SI{30}{^\circ})} \right) = \SI{68,96}{^\circ} \\
	F_{res} &= \frac{\SI{1,5}{Nm}}{1,3 \cdot \frac{\SI{4,773}{mm} + \SI{6}{mm}}{4} + \frac{\SI{5,35}{mm}}{2} \cdot \tan(\SI{3,405}{^\circ} + \SI{68,96}{^\circ})} = \SI{0,126}{N}
\end{align*}

Bei sechs Schrauben erhält man eine Gesamtkraft von $F_{ges} = \SI{0,755}{N}$. Das führt zu einer dichten Verbindung.
